...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640):
        ...
        images = [x for x in files if os.path.splitext(x)[-1].lower() in img_formats]      # 检查图片扩展名是否符合要求
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # batch index, 类似 [0, 0, .., 1, 1, .., 2, 2, ..], 代表属于第几个batch
        ...
        if self.rect:
            ...
            shapes = [[1, 1]] * nb
            for i in range(nb):     # 每个batch内的图片保持shape相同
                ari = ar[bi == i]
        ...
        for i, file in pbar:
            ...
            if l is not None and l.shape[0]:
                ...
                if np.unique(l, axis=0).shape[0] < l.shape[0]:  # duplicate rows, 即一张图片中出现了重复的label
            ...
            try:
                ...
                if os.path.isfile(label):
                    with open(label, 'r') as f:
                        # labels, l中的每个元素也是一个数组, l[i] = [cls, cx, cy, w, h]
    ...
    def __getitem__(self, index):
        ...
        # 上面中的两步都会将label由xywh->xyxy， 方便进行下面的augment， 但不会修改原始的self.labels
    ...
    def collate_fn(batch):
        ...
        # label: index, cls, cx, cy, w, h, index代表这是batch中第几张图片
