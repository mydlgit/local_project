...
def compute_loss(p, targets, model):  # predictions, targets, model
    """
        ...
        :param p: model output, List[torch.tensor*nl], nl = detector.nl, p[i].shape = (bs, na, h, w, (num_cls+5))
        :param targets: shape=(nt, 6), idx_in_batch, cls, cx, cy, w, h in order
        :param model:
        :return:
    """
    ...
    balance = [4.0, 1.0, 0.4] if np == 3 else [4.0, 1.0, 0.4, 0.1]  # P3-5 or P3-6, 不同的特征图对应的loss weight
    for i, pi in enumerate(p):  # layer index, layer predictions
        b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx
        tobj = torch.zeros_like(pi[..., 0], device=device)  # target obj, shape =  (bs, na, h, w)
        ...
        lobj += BCEobj(pi[..., 4], tobj) * balance[i]  # obj loss, pi[..., 4]代表相应位置上cls的score, shape=(bs, na, h, w)
...
def build_targets(p, targets, model):
    ...
    # : p is a list, length = detector.nl,
    ...
    targets = torch.cat((targets.repeat(na, 1, 1), ai[:, :, None]), 2)  # append anchor indices, shape:na*nt*(6+1)
    # targets[i, j, -1] = i, 代表第j个target此时对应第i个anchor
    ...
    for i in range(det.nl):     # 即有多少个detector head
        anchors = det.anchors[i]    # anchors.shape = (na, 2)
        # xyxy gain, p[i].shape = m*na*h*w*(num_cls+5), 这里h, w是相应特征图的size
        ...
        if nt:
            # Matches,
            """
            t[:, :, 4:6].shape = [na, nt, 2], anchors[:, None].shape=(na, 1, 2)
            筛选满足 1/hyp['anchor_t']<targets_wh/anchor_wh<hyp['anchor_t']的标签框
            这样做的目的是找到与该label匹配度好的anchor
            """
            r = t[:, :, 4:6] / anchors[:, None]  # wh ratio
            j = torch.max(r, 1. / r).max(2)[0] < model.hyp['anchor_t']  # compare
            # j = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n)=wh_iou(anchors(3,2), gwh(n,2))
            t = t[j]  # filter， shape=(M, 7), M为符合的gt box

            # Offsets
            gxy = t[:, 2:4]  # grid xy, 中心点坐标， 相对于左上角
            gxi = gain[[2, 3]] - gxy  # inverse， 相对于右下角的中心坐标

            """
            j, k, l, m 分别代表 相对与网格左上角x_offset<0.5(同时x>1), 相对于网格左上角y_offset<0.5(同时y>1),
            相对于网格右下角x_offset<0.5(同时x>1), 相对于网格右下角y_offset<0.5(同时y>1)的gt box。 (x, y相对于特征图左上角/右下角而言)
            可知一个gt box 可以满足多个条件
            eg. 中心点cen(1.4, 1.8)的gt box同时属于j, l
            即可以看作将一个网格(grid)分为四部分， 中心点位于其中一部分的gt box，不仅采用当前部分对应的anchor来回归，同时依赖相邻两部分的网格
            a--------b--------c
            |        |_ _:_ _ |  ,  cen位于左下部分，其中心点由b, e, f来回归。 目的是缓解网格效应
            |        |   :    | 
            d--------e--------f
             
            中心点回归范围由0~1, 变为-0.5～1.5, 由yolo.py中
                y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + self.grid[i].to(x[i].device)) * self.stride[i]
            也可以看出
            """
            j, k = ((gxy % 1. < g) & (gxy > 1.)).T      # gxy % 1. 即求小数位
            l, m = ((gxi % 1. < g) & (gxi > 1.)).T
            # 将四个部分的框与原始的gxy拼接在一起
            j = torch.stack((torch.ones_like(j), j, k, l, m))       # j.shape=(5, M)， 共有 N个元素为True

            t = t.repeat((5, 1, 1))[j]      # t.shape: (5, M, 7) -> (N, 7), N为筛选后的个数
            # 偏移量 shape: (1, M, 2) + (5, 1, 2) = (5, M, 2) -> (N, 2)
