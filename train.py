...
from utils.general import (
    ...
    check_git_status, check_img_size, increment_dir, print_mutation, plot_evolution, set_logging, plot_lr_scheduler)
...
def train(hyp, opt, device, tb_writer=None):
    logger.info(f'Hyperparameters {hyp}')       # 以 f开头表示在字符串内支持大括号内的python 表达式
    ...
    if pretrained:
        ...
        # 用已保存的参数来映射到新读取的结构， 因为可能model的结构来自opt.cfg, 与原始结构不能一一对应
    ...
    print('optimizer:', 'Adam' if opt.adam else 'SGD')
    if opt.adam:
    ...
    # plot_lr_scheduler(optimizer, scheduler, epochs)     # 绘制训练过程中learning rate变化曲线
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/TT100K.yaml', help='data.yaml path')
    ...
    parser.add_argument('--evolve', action='store_true', help='evolve hyperparameters', default=False)
    parser.add_argument('--bucket', type=str, default='', help='gsutil bucket')
    parser.add_argument('--cache-images', action='store_true', help='cache images for faster training')
    parser.add_argument('--name', default='yolov5x_TT100K', help='renames results.txt to results_name.txt if supplied')
    ...
    # Set DDP variables, 分布式训练参数设置
    opt.total_batch_size = opt.batch_size
    opt.world_size = int(os.environ['WORLD_SIZE']) if 'WORLD_SIZE' in os.environ else 1
    opt.global_rank = int(os.environ['RANK']) if 'RANK' in os.environ else -1
    set_logging(opt.global_rank)

    # Resume, 加载上次训练最后一次epoch的参数
    if opt.resume:
    ...
    # Evolve hyperparameters (optional), 通过遗传算法找到最优参数, （十分耗时）
    else:
